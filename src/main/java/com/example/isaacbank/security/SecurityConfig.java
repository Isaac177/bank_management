...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    // Injecting the data source into the security config.
    @Autowired
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
        ...
        auth.jdbcAuthentication()
                // A query that is being executed to get the user from the database.
...
//                .passwordEncoder(new BCryptPasswordEncoder().);
    ...
    // This is the configuration for the security.
    @Override
    protected void configure(HttpSecurity http) throws Exception {
        http.formLogin().loginPage("/login");
        http.authorizeRequests().antMatchers("/operations","/checkAccount").hasRole("USER");
